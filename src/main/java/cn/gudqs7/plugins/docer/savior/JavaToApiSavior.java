...
public class JavaToApiSavior extends BaseSavior {
    ...
    public String java2api(PsiClassReferenceType psiClassReferenceType, Project project) {
        ...
        convertPsiClassToMarkdown(project, goMap, psiClassReferenceType, clazzDesc, 1, null, "", false);
    ...
    private String java2apiCommon(Project project, PsiType psiType, String fieldName, String clazzTypeName, AnnotationHolder fieldAnnotation, boolean returnParam) {
        ...
        if (isJavaBaseType(typeName) || "Object".equals(typeName) || PsiUtil.isPsiTypeFromMap(psiType, project)) {
            level = 1;
        }
    ...
    private void convertPsiClassToMarkdown(Project project, Map<Integer, List<ParamInfo>> goMap, PsiClassReferenceType psiClassReferenceType, String clazzDesc, int level, String parentClazzTypeName, String fieldPrefix, boolean returnParam) {
        if (psiClassReferenceType == null) {
            return;
        }
        PsiClass psiClass = psiClassReferenceType.resolve();
        ...
        for (PsiField typeDeclaredField : typeDeclaredFields) {
            ...
            handlerByFieldInfo(project, goMap, level, clazzTypeName, levelOther, fieldName, psiFieldType, annotationHolder, psiClassReferenceType, returnParam, fieldPrefix);
    ...
    private void handlerByFieldInfo(Project project, Map<Integer, List<ParamInfo>> goMap, int level, String clazzTypeName, List<ParamLineInfo> levelOther, String fieldName, PsiType psiFieldType, AnnotationHolder annotationHolder, PsiClassReferenceType topClassReferenceType, boolean returnParam, String fieldPrefix) {
        if (topClassReferenceType != null) {
            PsiClass psiClass = topClassReferenceType.resolve();
            if (psiClass == null) {
                PsiUtil.handleSyntaxError(topClassReferenceType.getCanonicalText());
            }
            String qualifiedName = psiClass.getQualifiedName();
        ...
        Map<String, String> data = handlerByPsiType(project, goMap, level, fieldName, psiFieldType,
                annotationHolder, clazzTypeName, "%s", typeName, fieldPrefix, returnParam);
        if (data != null) {
    ...
    private Map<String, String> handlerByPsiType(Project project, Map<Integer, List<ParamInfo>> goMap, int level, String fieldName, PsiType psiFieldType, AnnotationHolder annotationHolder, String clazzTypeName, String typeNameFormat, String typeName, String fieldPrefix, boolean returnParam) {
        ApiModelProperty apiModelProperty = annotationHolder.getApiModelProperty();
        ...
        if (isArrayType) {
            ...
            return handlerByPsiType(project, goMap, level, fieldName, componentType, annotationHolder, clazzTypeName, typeFormat, realTypeName, fieldPrefix, returnParam);
        }
        ...
        if (isReferenceType) {
            ...
            if (realPsiType != null) {
                ...
                String typeFormat = String.format(typeNameFormat, "%s");
                return handlerByPsiType(project, goMap, level, fieldName, realPsiType, annotationHolder, clazzTypeName, typeFormat, realTypeName, fieldPrefix, returnParam);
            }
            ...
            if (PsiUtil.isPsiTypeFromList(psiFieldType, project)) {
                if (parameters.length > 0) {
                    ...
                    return handlerByPsiType(project, goMap, level, fieldName, elementType, annotationHolder, clazzTypeName, typeFormat, realTypeName, fieldPrefix, returnParam);
                } else {
                    return data;
                }
            }
            ...
            if (PsiUtil.isPsiTypeFromMap(psiFieldType, project)) {
                if (parameters.length > 1) {
                    ...
                    return handlerByPsiType(project, goMap, level, fieldName, valueType, annotationHolder, clazzTypeName, typeFormat, realTypeName, fieldPrefix, returnParam);
                } else {
                    return data;
                }
            }
            ...
            if (!PsiUtil.isPsiTypeFromParameter(psiFieldType)) {
                ...
                convertPsiClassToMarkdown(project, goMap, psiClassReferenceType, fieldDesc, level + 1, clazzTypeName, fieldPrefix0, returnParam);
            }
