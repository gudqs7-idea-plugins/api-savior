...
public class BaseTypeUtil {
    ...
    private static String randomDate(CommentInfo commentInfo) {
        Date now = new Date();
        String pattern = commentInfo.getSingleStr(MoreCommentTag.JSON_FORMAT.getTag(), "yyyy-MM-dd'T'HH:mm:ss.SSS+0000");
        pattern = commentInfo.getSingleStr(MoreCommentTag.DATE_FORMAT.getTag(), pattern);
    ...
    private static String randomString(CommentInfo commentInfo) {
        ...
        Boolean random = commentInfo.getSingleBool(MoreCommentTag.EXAMPLE_RANDOM.getTag(), false);
        Boolean guid = commentInfo.getSingleBool(MoreCommentTag.EXAMPLE_GUID.getTag(), false);
        if (guid) {
            if (notUsingRandom()) {
